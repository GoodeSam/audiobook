...
# 服务器上的 library/ 是管理员发布的内容库，rsync 时排除，永不覆盖。
#
...
echo "build dist/ (base=/) ..."
(cd "$ROOT" && DEPLOY_BASE=/ npx vite build)

echo "sync dist/ -> $HOST:$DEST (keeping server-side library/)"
rsync -az --delete --exclude 'library/' -e "ssh -i $KEY -o StrictHostKeyChecking=accept-new" \
  "$ROOT/dist/" "$HOST:$DEST/"

ssh -i "$KEY" "$HOST" 'nginx -t >/dev/null 2>&1 && systemctl reload nginx && echo nginx-reloaded'
echo "OK -> https://audiobook.tumei.online/"
